...
try:
    ...
    print("✅ 测试 1 完美通过：检索接口与心跳接口完全独立运行！")
...
if proc.poll() is not None:
    print("✅ 服务器已干净地自动退出 (Exit code 0)！进程无残留！")
else:
    print("❌ 测试 2 失败：服务器在 17 秒后依然存活！")
    proc.kill()
    sys.exit(1)
...
if proc2.poll() is not None:
    print("✅ 服务器在 60 秒极长宽限期后已自动退出，未变成僵尸进程！")
else:
    print("❌ 测试 3 失败：60 秒后服务器仍存活！")
    proc2.kill()
    sys.exit(1)

print("\n🎉 所有严格黑盒测试全部通过！系统机制堪称完美！")
