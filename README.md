...
### 📖 核心功能四：古籍原典载录语境全景透视（现代点校整理体例）
* **上下文全景展开**：出处卡片中点击 **`📜 原典前后文`**，即可展开本诗在古籍原书中的前文数行、命中诗行与后文数行；
* **文人典雅三段式排版**：严格区分「前文载录」（松烟淡墨）、「本篇实况」（朱红古雅聚焦卡片）与「后文续考」，版面条理井然；
* **古籍版刻噪点彻底清整**：
  * **伪折行智能缝合**：消除古籍雕版每列固定 20 字机械折行引起的诗句腰斩，实现文句自然平滑流转；
  * **双行小字注连缀**：OCR 双行注跨行腰斩（`)\n(`）自动缝合为完整注记，并采用典雅小字排印；
  * **清除杂乱符号**：彻底滤除 `# src:`、`# dating:` 等数据库元数据注释及 OCR 列断斜杠 `(一作/天)`；
  * **词律与叶码文人标识**：词曲韵位标记（`〔韵〕`）与叶码（`〔第N叶正/背〕`）化作精致书叶微标；
* **超长语境渐变蒙层与按需展开**：类书等长篇语境自动应用渐变羽化蒙层并标注总字数，支持平滑展开与收起；
* **检索词朱砂穿透高亮**：精准定位检索关键词，朱砂微晕穿透原典版面，便于快速对勘。
...
在项目根目录下**直接双击运行 [`启动系统.bat`](启动系统.bat)**：

* **环境智能自适应**：脚本自动检测环境，若检测到 Python 则直接以源码运行，若未安装 Python 则自动调用 [`dist/`](dist/) 目录下的免安装便携程序；
* **自动展卷就绪**：启动后系统将自动唤醒默认浏览器打开学术检索主界面（默认访问 `http://localhost:8080`）；
* 亦可直接进入 [`dist/`](dist/) 目录双击运行 **`souyun-index-windows.exe`**。
