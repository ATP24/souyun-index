...
### 📖 核心功能四：古籍原典载录语境全景透视
* **上下文全景展开**：出处卡片中点击 **`📜 原典前后文`**，即可展开本诗在古籍原书中的前文数行、命中诗行与后文数行；
* **朱笔批点式高亮**：正文命中部分采用传统朱砂句读风格（胭脂暖红下划线），与古籍前后文墨色自然区分；
* **古籍技术标记转译**：古籍数据库底层技术标记（如 `<pb:KR...-12a>`）智能转译为典雅易读的书叶标识（如 `〔第12叶正〕`）。
...
### 方式一：Windows 极速启动（双击即用 · 最优体验）

解压本仓库或从 Releases 下载的完整包后，在项目根目录下任选一种方式即可秒级启动：

* **【推荐】一键启动脚本**：直接双击根目录下的 **[`启动系统.bat`](启动系统.bat)**。系统将智能探测环境，无论是否安装 Python 均能自动就绪并自动在浏览器中打开学术主界面；
* **【网页入口】快捷启动网页**：双击根目录下的 **[`快捷启动.html`](快捷启动.html)**，自动探测本地服务状态并平滑跳转；
* **【桌面直达】创建桌面快捷方式**：双击运行 **[`创建桌面快捷方式.vbs`](创建桌面快捷方式.vbs)**，一键在电脑桌面上生成启动图标，日后在桌面双击即可使用；
* **【便携程序】独立可执行文件**：直接进入 [`dist/`](dist/) 目录双击运行 **`souyun-index-windows.exe`**。
...
├── 启动系统.bat                # Windows 一键极速双击启动脚本 (自动识别环境)
├── 快捷启动.html               # 桌面级快速探测与就绪入口
├── 快捷访问.url                # 本地服务网页直连快捷方式
├── 创建桌面快捷方式.vbs        # 一键在电脑桌面生成启动快捷方式
├── main.py                     # Python 开源标准官方入口模块
