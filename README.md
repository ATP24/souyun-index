...
### 方式一：Windows 免安装便携版（开箱即用 · 推荐）

直接下载本仓库或 Releases 中的可执行程序，无需配置 Python 环境：

1. 打开本仓库根目录下的 [`dist/`](dist/) 文件夹，或者访问 👉 **[GitHub Releases 下载页](https://github.com/ATP24/souyun-index/releases/latest)**；
2. 下载并解压 **`souyun-index-windows.exe`**；
3. **双击直接运行**：程序启动后将自动唤起默认浏览器，打开学术检索界面（默认访问 `http://localhost:8080`）。
